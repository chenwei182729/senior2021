...
public class MyConfiguration {
    ...
    @SuppressWarnings("rawtypes")
    public MapperBean readAnnotation(Class interfaceName) {
        Method[] methods = interfaceName.getDeclaredMethods();
        MapperBean mapper = new MapperBean();
        if (methods == null) {
            return mapper;
        }
        mapper.setInterfaceName(interfaceName.getName());
        ...
        for (Method method : methods) {
            ...
            fun.setSql(value);
            fun.setSqltype("select");
            fun.setResultType(method.getReturnType());
            fun.setFuncName(method.getName());
            list.add(fun);
        }
        mapper.setList(list);
